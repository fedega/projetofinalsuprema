<%--== Handlers ==--%> <%--ManterCliente Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-9B73B966
    public class ManterClienteServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_cliente Record Handler Head @2-0D304AE8
    public class ManterClientetbl_clienteRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-257E042E
    Page ManterClienteModel = (Page)request.getAttribute("ManterCliente_page");
    Page ManterClienteParent = (Page)request.getAttribute("ManterClienteParent");
    if (ManterClienteModel == null) {
        PageController ManterClienteCntr = new PageController(request, response, application, "/ManterCliente.xml" );
        ManterClienteModel = ManterClienteCntr.getPage();
        ManterClienteModel.setRelativePath("./");
        //if (ManterClienteParent != null) {
            //if (!ManterClienteParent.getChild(ManterClienteModel.getName()).isVisible()) return;
        //}
        ((Record)ManterClienteModel.getChild("tbl_cliente")).addRecordListener(new ManterClientetbl_clienteRecordHandler());
        ((TextBox)((Record)ManterClienteModel.getChild("tbl_cliente")).getChild("Nome")).addValidationListener(new tbl_clienteNomeTextBoxHandler());
        ((TextBox)((Record)ManterClienteModel.getChild("tbl_cliente")).getChild("Data_Nasc")).addValidationListener(new tbl_clienteData_NascTextBoxHandler());
        ((TextBox)((Record)ManterClienteModel.getChild("tbl_cliente")).getChild("Nome_U")).addValidationListener(new tbl_clienteNome_UTextBoxHandler());
        ((TextBox)((Record)ManterClienteModel.getChild("tbl_cliente")).getChild("Senha_U")).addValidationListener(new tbl_clienteSenha_UTextBoxHandler());
        ((TextBox)((Record)ManterClienteModel.getChild("tbl_cliente")).getChild("Nacionalidade")).addValidationListener(new tbl_clienteNacionalidadeTextBoxHandler());
        ((TextBox)((Record)ManterClienteModel.getChild("tbl_cliente")).getChild("Endereco")).addValidationListener(new tbl_clienteEnderecoTextBoxHandler());
        ManterClienteCntr.process();
%>
        <% request.setAttribute("HeaderParent", ManterClienteModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", ManterClienteModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (ManterClienteParent == null) {
            ManterClienteModel.setCookies();
            if (ManterClienteModel.redirect()) return;
        } else {
            ManterClienteModel.redirect();
        }
    }
//End Processing
