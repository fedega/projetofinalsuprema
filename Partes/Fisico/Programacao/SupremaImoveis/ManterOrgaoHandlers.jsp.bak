<%--== Handlers ==--%> <%--ManterOrgao Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-AA6B0925
    public class ManterOrgaoServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_orgaoemissor Record Handler Head @2-6CC3DFC8
    public class ManterOrgaotbl_orgaoemissorRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-70AEEB60
    Page ManterOrgaoModel = (Page)request.getAttribute("ManterOrgao_page");
    Page ManterOrgaoParent = (Page)request.getAttribute("ManterOrgaoParent");
    if (ManterOrgaoModel == null) {
        PageController ManterOrgaoCntr = new PageController(request, response, application, "/ManterOrgao.xml" );
        ManterOrgaoModel = ManterOrgaoCntr.getPage();
        ManterOrgaoModel.setRelativePath("./");
        //if (ManterOrgaoParent != null) {
            //if (!ManterOrgaoParent.getChild(ManterOrgaoModel.getName()).isVisible()) return;
        //}
        ((Record)ManterOrgaoModel.getChild("tbl_orgaoemissor")).addRecordListener(new ManterOrgaotbl_orgaoemissorRecordHandler());
        ((TextBox)((Record)ManterOrgaoModel.getChild("tbl_orgaoemissor")).getChild("Sigla")).addValidationListener(new tbl_orgaoemissorSiglaTextBoxHandler());
        ManterOrgaoCntr.process();
%>
        <% request.setAttribute("HeaderParent", ManterOrgaoModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", ManterOrgaoModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (ManterOrgaoParent == null) {
            ManterOrgaoModel.setCookies();
            if (ManterOrgaoModel.redirect()) return;
        } else {
            ManterOrgaoModel.redirect();
        }
    }
//End Processing
