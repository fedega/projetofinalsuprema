<%--== Handlers ==--%> <%--tbl_tipo_solici_maint Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-7685E502
    public class tbl_tipo_solici_maintServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_tipo_solicitacao Record Handler Head @2-0F39C521
    public class tbl_tipo_solici_mainttbl_tipo_solicitacaoRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-5A9D94BE
    Page tbl_tipo_solici_maintModel = (Page)request.getAttribute("tbl_tipo_solici_maint_page");
    Page tbl_tipo_solici_maintParent = (Page)request.getAttribute("tbl_tipo_solici_maintParent");
    if (tbl_tipo_solici_maintModel == null) {
        PageController tbl_tipo_solici_maintCntr = new PageController(request, response, application, "/tbl_tipo_solici_maint.xml" );
        tbl_tipo_solici_maintModel = tbl_tipo_solici_maintCntr.getPage();
        tbl_tipo_solici_maintModel.setRelativePath("./");
        ...
        //}
        ((Record)tbl_tipo_solici_maintModel.getChild("tbl_tipo_solicitacao")).addRecordListener(new tbl_tipo_solici_mainttbl_tipo_solicitacaoRecordHandler());
        tbl_tipo_solici_maintCntr.process();
%>
        <% request.setAttribute("HeaderParent", tbl_tipo_solici_maintModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", tbl_tipo_solici_maintModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (tbl_tipo_solici_maintParent == null) {
            tbl_tipo_solici_maintModel.setCookies();
            if (tbl_tipo_solici_maintModel.redirect()) return;
        } else {
            tbl_tipo_solici_maintModel.redirect();
        }
    }
//End Processing
