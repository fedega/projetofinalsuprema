<%--== Handlers ==--%> <%--tbl_estado_maint Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-CCF71EF0
    public class tbl_estado_maintServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_estado Record Handler Head @2-D7D4A902
    public class tbl_estado_mainttbl_estadoRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-C7F6419D
    Page tbl_estado_maintModel = (Page)request.getAttribute("tbl_estado_maint_page");
    Page tbl_estado_maintParent = (Page)request.getAttribute("tbl_estado_maintParent");
    if (tbl_estado_maintModel == null) {
        PageController tbl_estado_maintCntr = new PageController(request, response, application, "/tbl_estado_maint.xml" );
        tbl_estado_maintModel = tbl_estado_maintCntr.getPage();
        tbl_estado_maintModel.setRelativePath("./");
        //if (tbl_estado_maintParent != null) {
            //if (!tbl_estado_maintParent.getChild(tbl_estado_maintModel.getName()).isVisible()) return;
        //}
        ((Record)tbl_estado_maintModel.getChild("tbl_estado")).addRecordListener(new tbl_estado_mainttbl_estadoRecordHandler());
        ((TextBox)((Record)tbl_estado_maintModel.getChild("tbl_estado")).getChild("UF")).addValidationListener(new tbl_estadoUFTextBoxHandler());
        ((TextBox)((Record)tbl_estado_maintModel.getChild("tbl_estado")).getChild("Nome")).addValidationListener(new tbl_estadoNomeTextBoxHandler());
        tbl_estado_maintCntr.process();
%>
        <% request.setAttribute("HeaderParent", tbl_estado_maintModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", tbl_estado_maintModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (tbl_estado_maintParent == null) {
            tbl_estado_maintModel.setCookies();
            if (tbl_estado_maintModel.redirect()) return;
        } else {
            tbl_estado_maintModel.redirect();
        }
    }
//End Processing
