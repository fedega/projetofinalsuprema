<%--== Handlers ==--%> <%--ManterFiador Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-CF406014
    public class ManterFiadorServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_fiador Record Handler Head @2-C3947C49
    public class ManterFiadortbl_fiadorRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-98755C1B
    Page ManterFiadorModel = (Page)request.getAttribute("ManterFiador_page");
    Page ManterFiadorParent = (Page)request.getAttribute("ManterFiadorParent");
    if (ManterFiadorModel == null) {
        PageController ManterFiadorCntr = new PageController(request, response, application, "/ManterFiador.xml" );
        ManterFiadorModel = ManterFiadorCntr.getPage();
        ManterFiadorModel.setRelativePath("./");
        //if (ManterFiadorParent != null) {
            //if (!ManterFiadorParent.getChild(ManterFiadorModel.getName()).isVisible()) return;
        //}
        ((Record)ManterFiadorModel.getChild("tbl_fiador")).addRecordListener(new ManterFiadortbl_fiadorRecordHandler());
        ((TextBox)((Record)ManterFiadorModel.getChild("tbl_fiador")).getChild("Nome")).addValidationListener(new tbl_fiadorNomeTextBoxHandler());
        ((TextBox)((Record)ManterFiadorModel.getChild("tbl_fiador")).getChild("Data_Nasc")).addValidationListener(new tbl_fiadorData_NascTextBoxHandler());
        ((TextBox)((Record)ManterFiadorModel.getChild("tbl_fiador")).getChild("Nome_U")).addValidationListener(new tbl_fiadorNome_UTextBoxHandler());
        ((TextBox)((Record)ManterFiadorModel.getChild("tbl_fiador")).getChild("Senha_U")).addValidationListener(new tbl_fiadorSenha_UTextBoxHandler());
        ((TextBox)((Record)ManterFiadorModel.getChild("tbl_fiador")).getChild("Nacionalidaade")).addValidationListener(new tbl_fiadorNacionalidaadeTextBoxHandler());
        ((TextBox)((Record)ManterFiadorModel.getChild("tbl_fiador")).getChild("Endereco")).addValidationListener(new tbl_fiadorEnderecoTextBoxHandler());
        ManterFiadorCntr.process();
%>
        <% request.setAttribute("HeaderParent", ManterFiadorModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", ManterFiadorModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (ManterFiadorParent == null) {
            ManterFiadorModel.setCookies();
            if (ManterFiadorModel.redirect()) return;
        } else {
            ManterFiadorModel.redirect();
        }
    }
//End Processing
