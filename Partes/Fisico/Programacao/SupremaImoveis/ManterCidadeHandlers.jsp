<%--== Handlers ==--%> <%--ManterCidade Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-78A91885
    public class ManterCidadeServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_cidade Record Handler Head @2-A41CCA66
    public class ManterCidadetbl_cidadeRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-3755E7D2
    Page ManterCidadeModel = (Page)request.getAttribute("ManterCidade_page");
    Page ManterCidadeParent = (Page)request.getAttribute("ManterCidadeParent");
    if (ManterCidadeModel == null) {
        PageController ManterCidadeCntr = new PageController(request, response, application, "/ManterCidade.xml" );
        ManterCidadeModel = ManterCidadeCntr.getPage();
        ManterCidadeModel.setRelativePath("./");
        //if (ManterCidadeParent != null) {
            //if (!ManterCidadeParent.getChild(ManterCidadeModel.getName()).isVisible()) return;
        //}
        ((Record)ManterCidadeModel.getChild("tbl_cidade")).addRecordListener(new ManterCidadetbl_cidadeRecordHandler());
        ((TextBox)((Record)ManterCidadeModel.getChild("tbl_cidade")).getChild("Nome")).addValidationListener(new tbl_cidadeNomeTextBoxHandler());
        ((ListBox)((Record)ManterCidadeModel.getChild("tbl_cidade")).getChild("Cod_Estado")).addValidationListener(new tbl_cidadeCod_EstadoListBoxHandler());
        ManterCidadeCntr.process();
%>
        <% request.setAttribute("HeaderParent", ManterCidadeModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", ManterCidadeModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (ManterCidadeParent == null) {
            ManterCidadeModel.setCookies();
            if (ManterCidadeModel.redirect()) return;
        } else {
            ManterCidadeModel.redirect();
        }
    }
//End Processing
