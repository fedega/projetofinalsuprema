<%--== Handlers ==--%> <%--tbl_tipo_causul_maint Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-26FB518F
    public class tbl_tipo_causul_maintServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_tipo_causula Record Handler Head @2-6F71647C
    public class tbl_tipo_causul_mainttbl_tipo_causulaRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-2E176315
    Page tbl_tipo_causul_maintModel = (Page)request.getAttribute("tbl_tipo_causul_maint_page");
    Page tbl_tipo_causul_maintParent = (Page)request.getAttribute("tbl_tipo_causul_maintParent");
    if (tbl_tipo_causul_maintModel == null) {
        PageController tbl_tipo_causul_maintCntr = new PageController(request, response, application, "/tbl_tipo_causul_maint.xml" );
        tbl_tipo_causul_maintModel = tbl_tipo_causul_maintCntr.getPage();
        tbl_tipo_causul_maintModel.setRelativePath("./");
        ...
        //}
        ((Record)tbl_tipo_causul_maintModel.getChild("tbl_tipo_causula")).addRecordListener(new tbl_tipo_causul_mainttbl_tipo_causulaRecordHandler());
        ((TextBox)((Record)tbl_tipo_causul_maintModel.getChild("tbl_tipo_causula")).getChild("Tipo")).addValidationListener(new tbl_tipo_causulaTipoTextBoxHandler());
        tbl_tipo_causul_maintCntr.process();
%>
        <% request.setAttribute("HeaderParent", tbl_tipo_causul_maintModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", tbl_tipo_causul_maintModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (tbl_tipo_causul_maintParent == null) {
            tbl_tipo_causul_maintModel.setCookies();
            if (tbl_tipo_causul_maintModel.redirect()) return;
        } else {
            tbl_tipo_causul_maintModel.redirect();
        }
    }
//End Processing
