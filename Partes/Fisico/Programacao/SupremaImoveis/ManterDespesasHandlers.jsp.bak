<%--== Handlers ==--%> <%--ManterDespesas Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-99C3D7FA
    public class ManterDespesasServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_despesas Record Handler Head @2-A543995C
    public class ManterDespesastbl_despesasRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-D12387F5
    Page ManterDespesasModel = (Page)request.getAttribute("ManterDespesas_page");
    Page ManterDespesasParent = (Page)request.getAttribute("ManterDespesasParent");
    if (ManterDespesasModel == null) {
        PageController ManterDespesasCntr = new PageController(request, response, application, "/ManterDespesas.xml" );
        ManterDespesasModel = ManterDespesasCntr.getPage();
        ManterDespesasModel.setRelativePath("./");
        //if (ManterDespesasParent != null) {
            //if (!ManterDespesasParent.getChild(ManterDespesasModel.getName()).isVisible()) return;
        //}
        ((Record)ManterDespesasModel.getChild("tbl_despesas")).addRecordListener(new ManterDespesastbl_despesasRecordHandler());
        ((TextBox)((Record)ManterDespesasModel.getChild("tbl_despesas")).getChild("Valor")).addValidationListener(new tbl_despesasValorTextBoxHandler());
        ((TextBox)((Record)ManterDespesasModel.getChild("tbl_despesas")).getChild("Cod_Aluguel")).addValidationListener(new tbl_despesasCod_AluguelTextBoxHandler());
        ManterDespesasCntr.process();
%>
        <% request.setAttribute("HeaderParent", ManterDespesasModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", ManterDespesasModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (ManterDespesasParent == null) {
            ManterDespesasModel.setCookies();
            if (ManterDespesasModel.redirect()) return;
        } else {
            ManterDespesasModel.redirect();
        }
    }
//End Processing
