<%--== Handlers ==--%> <%--tbl_preferencia_maint Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-E32229A2
    public class tbl_preferencia_maintServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_preferencia Record Handler Head @2-6A69B47B
    public class tbl_preferencia_mainttbl_preferenciaRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-6838F721
    Page tbl_preferencia_maintModel = (Page)request.getAttribute("tbl_preferencia_maint_page");
    Page tbl_preferencia_maintParent = (Page)request.getAttribute("tbl_preferencia_maintParent");
    if (tbl_preferencia_maintModel == null) {
        PageController tbl_preferencia_maintCntr = new PageController(request, response, application, "/tbl_preferencia_maint.xml" );
        tbl_preferencia_maintModel = tbl_preferencia_maintCntr.getPage();
        tbl_preferencia_maintModel.setRelativePath("./");
        ...
        //}
        ((Record)tbl_preferencia_maintModel.getChild("tbl_preferencia")).addRecordListener(new tbl_preferencia_mainttbl_preferenciaRecordHandler());
        ((TextBox)((Record)tbl_preferencia_maintModel.getChild("tbl_preferencia")).getChild("Cod_Cliente")).addValidationListener(new tbl_preferenciaCod_ClienteTextBoxHandler());
        tbl_preferencia_maintCntr.process();
%>
        <% request.setAttribute("HeaderParent", tbl_preferencia_maintModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", tbl_preferencia_maintModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (tbl_preferencia_maintParent == null) {
            tbl_preferencia_maintModel.setCookies();
            if (tbl_preferencia_maintModel.redirect()) return;
        } else {
            tbl_preferencia_maintModel.redirect();
        }
    }
//End Processing
