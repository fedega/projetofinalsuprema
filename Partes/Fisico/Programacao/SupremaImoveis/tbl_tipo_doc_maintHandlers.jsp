<%--== Handlers ==--%> <%--tbl_tipo_doc_maint Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-D1303201
    public class tbl_tipo_doc_maintServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_tipo_doc Record Handler Head @2-96122811
    public class tbl_tipo_doc_mainttbl_tipo_docRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-362B224D
    Page tbl_tipo_doc_maintModel = (Page)request.getAttribute("tbl_tipo_doc_maint_page");
    Page tbl_tipo_doc_maintParent = (Page)request.getAttribute("tbl_tipo_doc_maintParent");
    if (tbl_tipo_doc_maintModel == null) {
        PageController tbl_tipo_doc_maintCntr = new PageController(request, response, application, "/tbl_tipo_doc_maint.xml" );
        tbl_tipo_doc_maintModel = tbl_tipo_doc_maintCntr.getPage();
        tbl_tipo_doc_maintModel.setRelativePath("./");
        //if (tbl_tipo_doc_maintParent != null) {
            //if (!tbl_tipo_doc_maintParent.getChild(tbl_tipo_doc_maintModel.getName()).isVisible()) return;
        //}
        ((Record)tbl_tipo_doc_maintModel.getChild("tbl_tipo_doc")).addRecordListener(new tbl_tipo_doc_mainttbl_tipo_docRecordHandler());
        tbl_tipo_doc_maintCntr.process();
%>
        <% request.setAttribute("HeaderParent", tbl_tipo_doc_maintModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", tbl_tipo_doc_maintModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (tbl_tipo_doc_maintParent == null) {
            tbl_tipo_doc_maintModel.setCookies();
            if (tbl_tipo_doc_maintModel.redirect()) return;
        } else {
            tbl_tipo_doc_maintModel.redirect();
        }
    }
//End Processing
