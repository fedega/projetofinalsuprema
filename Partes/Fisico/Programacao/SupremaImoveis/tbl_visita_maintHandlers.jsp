<%--== Handlers ==--%> <%--tbl_visita_maint Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-59017B1E
    public class tbl_visita_maintServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_visita Record Handler Head @2-BDC68645
    public class tbl_visita_mainttbl_visitaRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-98F0F7F1
    Page tbl_visita_maintModel = (Page)request.getAttribute("tbl_visita_maint_page");
    Page tbl_visita_maintParent = (Page)request.getAttribute("tbl_visita_maintParent");
    if (tbl_visita_maintModel == null) {
        PageController tbl_visita_maintCntr = new PageController(request, response, application, "/tbl_visita_maint.xml" );
        tbl_visita_maintModel = tbl_visita_maintCntr.getPage();
        tbl_visita_maintModel.setRelativePath("./");
        //if (tbl_visita_maintParent != null) {
            //if (!tbl_visita_maintParent.getChild(tbl_visita_maintModel.getName()).isVisible()) return;
        //}
        ((Record)tbl_visita_maintModel.getChild("tbl_visita")).addRecordListener(new tbl_visita_mainttbl_visitaRecordHandler());
        tbl_visita_maintCntr.process();
%>
        <% request.setAttribute("HeaderParent", tbl_visita_maintModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", tbl_visita_maintModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (tbl_visita_maintParent == null) {
            tbl_visita_maintModel.setCookies();
            if (tbl_visita_maintModel.redirect()) return;
        } else {
            tbl_visita_maintModel.redirect();
        }
    }
//End Processing
