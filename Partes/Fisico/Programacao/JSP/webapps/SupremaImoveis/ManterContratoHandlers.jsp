<%--== Handlers ==--%> <%--ManterContrato Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-04AEA27A
    public class ManterContratoServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_contrato Record Handler Head @2-64690A49
    public class ManterContratotbl_contratoRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-78A44C4C
    Page ManterContratoModel = (Page)request.getAttribute("ManterContrato_page");
    Page ManterContratoParent = (Page)request.getAttribute("ManterContratoParent");
    if (ManterContratoModel == null) {
        PageController ManterContratoCntr = new PageController(request, response, application, "/ManterContrato.xml" );
        ManterContratoModel = ManterContratoCntr.getPage();
        ManterContratoModel.setRelativePath("./");
        //if (ManterContratoParent != null) {
            //if (!ManterContratoParent.getChild(ManterContratoModel.getName()).isVisible()) return;
        //}
        ((Record)ManterContratoModel.getChild("tbl_contrato")).addRecordListener(new ManterContratotbl_contratoRecordHandler());
        ManterContratoCntr.process();
%>
        <% request.setAttribute("HeaderParent", ManterContratoModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", ManterContratoModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (ManterContratoParent == null) {
            ManterContratoModel.setCookies();
            if (ManterContratoModel.redirect()) return;
        } else {
            ManterContratoModel.redirect();
        }
    }
//End Processing
