<%--== Handlers ==--%> <%--ManterVenda Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-B8DD8F20
    public class ManterVendaServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_venda Record Handler Head @2-7DB7F7CE
    public class ManterVendatbl_vendaRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-9408B46F
    Page ManterVendaModel = (Page)request.getAttribute("ManterVenda_page");
    Page ManterVendaParent = (Page)request.getAttribute("ManterVendaParent");
    if (ManterVendaModel == null) {
        PageController ManterVendaCntr = new PageController(request, response, application, "/ManterVenda.xml" );
        ManterVendaModel = ManterVendaCntr.getPage();
        ManterVendaModel.setRelativePath("./");
        //if (ManterVendaParent != null) {
            //if (!ManterVendaParent.getChild(ManterVendaModel.getName()).isVisible()) return;
        //}
        ((Record)ManterVendaModel.getChild("tbl_venda")).addRecordListener(new ManterVendatbl_vendaRecordHandler());
        ManterVendaCntr.process();
%>
        <% request.setAttribute("HeaderParent", ManterVendaModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", ManterVendaModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (ManterVendaParent == null) {
            ManterVendaModel.setCookies();
            if (ManterVendaModel.redirect()) return;
        } else {
            ManterVendaModel.redirect();
        }
    }
//End Processing
