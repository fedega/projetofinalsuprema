<%--== Handlers ==--%> <%--ManterAluguel Opening Initialization directive @1-A0E75F14--%><%!
...
//Feature checker Head @1-EB2C0635
    public class ManterAluguelServiceChecker implements com.codecharge.features.IServiceChecker {
...
//tbl_aluguel Record Handler Head @2-9DEB95AB
    public class ManterAlugueltbl_aluguelRecordHandler implements RecordListener, RecordDataObjectListener {
...
//Processing @1-38CC84C6
    Page ManterAluguelModel = (Page)request.getAttribute("ManterAluguel_page");
    Page ManterAluguelParent = (Page)request.getAttribute("ManterAluguelParent");
    if (ManterAluguelModel == null) {
        PageController ManterAluguelCntr = new PageController(request, response, application, "/ManterAluguel.xml" );
        ManterAluguelModel = ManterAluguelCntr.getPage();
        ManterAluguelModel.setRelativePath("./");
        //if (ManterAluguelParent != null) {
            //if (!ManterAluguelParent.getChild(ManterAluguelModel.getName()).isVisible()) return;
        //}
        ((Record)ManterAluguelModel.getChild("tbl_aluguel")).addRecordListener(new ManterAlugueltbl_aluguelRecordHandler());
        ManterAluguelCntr.process();
%>
        <% request.setAttribute("HeaderParent", ManterAluguelModel); %>
        <%{%><%@include file="/HeaderHandlers.jsp"%><%}%>
        <% request.setAttribute("FooterParent", ManterAluguelModel); %>
        <%{%><%@include file="/FooterHandlers.jsp"%><%}%>
<%
        if (ManterAluguelParent == null) {
            ManterAluguelModel.setCookies();
            if (ManterAluguelModel.redirect()) return;
        } else {
            ManterAluguelModel.redirect();
        }
    }
//End Processing
